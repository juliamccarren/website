...
function updateNeuralIndexStrip(songs) {
    ...
    if (!strip) {
        ...
        strip.addEventListener('touchstart', (e) => {
            // Wenn der Strip schmal ist (15px), öffne ihn
            if (strip.offsetWidth < 50) {
                strip.classList.add('mobile-expanded');
            } else if (e.target === strip) {
                // Wenn man auf den leeren Bereich im offenen Strip tippt, schließe ihn
...
function setupArchiveObserver() {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            // entry.isIntersecting reicht hier oft aus
            if (entry.isIntersecting) {
                strip.classList.add('visible');
            } else {
                strip.classList.remove('visible');
                // Sicherstellen, dass er auch einklappt
                strip.classList.remove('mobile-expanded');
            }
        });
    }, { 
        threshold: 0, // Reagiert sofort, wenn das Archiv auch nur 1px sichtbar ist
        rootMargin: "-10% 0px -10% 0px" // Puffer für oben und unten
    });
