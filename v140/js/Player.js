...
class NeuralPlayer {
    ...
    initEventListeners() {
        ...
        if (this.seekSlider) {
            ...
            // Neu: Sperre für Mobile/Touch beim Ziehen
            ...
            this.seekSlider.addEventListener('pointerup', endSeek);
            this.seekSlider.addEventListener('touchstart', startSeek, { passive: true });
            this.seekSlider.addEventListener('touchend', endSeek);
        }
    }
    ...
    toggle() {
        if (this.audio.paused) {
            // Falls bereits eine Audioquelle existiert, einfach an der pausierten Stelle fortsetzen
            if (this.audio.src && this.audio.src !== window.location.href) {
                this.play();
            } else {
                // Nur wenn noch gar keine Quelle geladen war, neu anfordern
                this.playCurrent();
            }
        } else {
            this.pause();
    ...
    */
     ...
     */
    handleTimeUpdate() {
        if (this.audio.duration && !this.isSeeking) { // <-- !this.isSeeking ergänzen
     ...
     */
    async updateMediaSessionOld(song) {
    ...
    async updateMediaSession(song) {
        if ('mediaSession' in navigator) {
            ...
            // 1. Dynamischer Cache-Name aus VersionCore
            const currentCacheName = `julia-site-v${VersionCore.info.number}`;
            
            // 2. Pfade auflösen (relativ zum Root/Versions-Ordner)
            const basePath = window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/'));
            const artworkUrl = `${basePath}/../artwork/${song.id}.webp`;
            const fallbackUrl = `${basePath}/../images/Hero_square.webp`;
            ...
            try {
                ...
                // SCHRITT 1: Versuch, das spezifische Artwork aus dem CACHE zu laden
                const cachedResponse = await cache.match(artworkUrl);
                if (cachedResponse) {
                    const blob = await cachedResponse.blob();
                    finalArtworkUrl = URL.createObjectURL(blob);
                    console.log(`SYSTEM: MediaSession using CACHED artwork for ${song.id}`);
                } 
                else {
                    // SCHRITT 2: Wenn nicht im Cache, Versuch über das INTERNET (Fetch)
                    try {
                        const internetResponse = await fetch(artworkUrl, { method: 'HEAD' });
                        if (internetResponse.ok) {
                            finalArtworkUrl = artworkUrl;
                            console.log(`SYSTEM: MediaSession using LIVE artwork for ${song.id}`);
                        } else {
                            throw new Error('Artwork not on server');
                        }
                    } catch (fetchError) {
                        // SCHRITT 3: INTERNET schlägt fehl (Offline/404) -> LOKALES FALLBACK
                        console.warn(`SYSTEM: Fetch failed, using fallback for ${song.id}`);
                        finalArtworkUrl = fallbackUrl;
                    }
                }
            } catch (e) {
                // Absolutes Sicherheitsnetz, falls alles oben kracht
                finalArtworkUrl = fallbackUrl;
            }
            ...
            // Handler für Steuerung (Hardware-Tasten / Auto-Display)
            // Handler für die Steuerung (Hardware-Tasten / Auto-Display)
            navigator.mediaSession.setActionHandler('play', () => this.play());
            navigator.mediaSession.setActionHandler('pause', () => this.pause());
            navigator.mediaSession.setActionHandler('previoustrack', () => this.previous());
    ...
    onSeekInput() {
        this.isSeeking = true;
        if (this.audio.duration && !isNaN(this.audio.duration)) {
            const val = parseFloat(this.seekSlider.value);
