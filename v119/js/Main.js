// --- GLOBAL INSTANCES ---
// Deklariere sie nur, aber weise sie noch nicht zu!
let songService;
let archiveCollection;
let featureCollection;
let podcastCollection;
let player;
let director;
let drumPicker;
let diaryService;
let cacheName = 'julia-music-v1'; // Einheitlicher Cache-Name für die gesamte App

// Global scroll state for modal management
let scrollPosition = 0;

// --- SHARED UI FUNCTIONS ---
// These bridge the gap between HTML onclick attributes and the class instances

/**
 * Führt ein manuelles Update durch, indem es die VersionCore im Root prüft
 * und zur Switch-Seite (Memory-Cache-Buster) navigiert.
 */
async function triggerManualUpdate(event) {
    if (event) event.preventDefault();
    
    // Visuelles Feedback im Menü
    const updateLink = event.currentTarget;
    const originalText = updateLink.innerHTML;
    updateLink.innerHTML = '<span>Checking...</span> <i data-lucide="loader" class="animate-spin" size="12"></i>';
    if (window.lucide) lucide.createIcons();

    try {
        /**
         * PFAD-LOGIK FÜR LOKAL & LIVE:
         * Wir nutzen "../../", um aus /v49/subfolder/ auszubrechen und zum Projekt-Root zu gelangen.
         * Das ist sicherer als window.location.origin, da origin lokale Unterordner ignoriert.
         */
        const relativeRoot = "../../";
        const versionUrl = `${relativeRoot}js/VersionCore.js?t=${Date.now()}`;
        
        // Fetch mit Cache-Bust, um die echte Datei vom Server/Disk zu erzwingen
        const response = await fetch(versionUrl, {
            cache: 'no-store',
            headers: { 'Cache-Control': 'no-cache' }
        });
        
        if (!response.ok) throw new Error("Root VersionCore nicht gefunden");
        
        const scriptText = await response.text();
        
        // Extrahiere die Nummer (z.B. "50") aus der statischen Klasse
        const match = scriptText.match(/"number":\s*"(\d+)"/);
        
        if (match && match[1]) {
            const newVersion = match[1];
            
            /**
             * REDIRECT ZUR SWITCH-SEITE:
             * Die Datei liegt im GUID-Ordner im Projekt-Root. 
             * Wir nutzen wieder den relativen Pfad "../../", um sicherzustellen, 
             * dass wir nicht bei "Cannot GET /..." landen.
             */
            const switchPath = "745596f4-2947-4d89-955f-f4148e07d22a/804b0424-9932-4e10-9874-0d2980fe87a6.html";
            const targetUrl = relativeRoot + switchPath;
            
            console.log(`SYSTEM: Root version ${newVersion} detected. Redirecting to Switcher...`);
            
            // Redirect ausführen
            window.location.href = targetUrl;
        } else {
            throw new Error("VersionCore Format ungültig");
        }
    } catch (error) {
        console.error("Manual Update failed:", error);
        updateLink.innerHTML = '<span>Update Error</span> <i data-lucide="alert-circle" size="12"></i>';
        if (window.lucide) lucide.createIcons();
        
        // Reset nach Fehlermeldung
        setTimeout(() => {
            updateLink.innerHTML = originalText;
            if (window.lucide) lucide.createIcons();
        }, 3000);
    }
}

/**
 * Sortiert die aktuellen Songs im Archiv-Grid alphabetisch.
 * @param {string} direction - 'asc' für A-Z, 'desc' für Z-A
 */
function sortArchiveSongs(direction) {
    if (!archiveCollection || !archiveCollection.songs.length) return;
    
    const sortedSongs = [...archiveCollection.songs];
    
    sortedSongs.sort((a, b) => {
        const titleA = a.title.toLowerCase();
        const titleB = b.title.toLowerCase();
        if (direction === 'asc') {
            return titleA.localeCompare(titleB);
        } else {
            return titleB.localeCompare(titleA);
        }
    });
    
    archiveCollection.update(sortedSongs);

    updateNeuralIndexStrip(sortedSongs);
    
    // Haptisches Feedback
    if (window.navigator.vibrate) window.navigator.vibrate(5);
}

/**
 * Sucht in der aktuellen Archiv-Auswahl und selektiert den ersten Treffer.
 * Deselektiert dabei alle anderen Kacheln und scrollt zum Ergebnis.
 * @param {string} query - Der Suchbegriff
 */
function handleArchiveSearch(query) {
    if (!archiveCollection || !archiveCollection.container || !archiveCollection.songs.length) return;
    
    const cards = Array.from(archiveCollection.container.children);
    // Definition der Klassen für das Highlighting (entspricht deinem Design-System)
    const highlightClasses = ['ring-2', 'ring-fuchsia-500', 'bg-neutral-900/60'];
    
    // Zuerst alle Kacheln deselektieren
    cards.forEach(card => {
        card.classList.remove(...highlightClasses);
    });

    // Wenn das Suchfeld leer ist, beenden wir die Funktion
    if (!query || query.trim() === "") return;

    const term = query.toLowerCase();
    const matchIndex = archiveCollection.songs.findIndex(song => 
        song.title.toLowerCase().includes(term) //|| 
       // song.background.toLowerCase().includes(term)
    );
    
    if (matchIndex !== -1 && cards[matchIndex]) {
        const targetCard = cards[matchIndex];
        
        // Den Treffer visuell hervorheben
        targetCard.classList.add(...highlightClasses);
        
        // Sanft zum Treffer scrollen
        targetCard.scrollIntoView({ 
            behavior: 'smooth', 
            block: 'center' 
        });
    }
}

/**
 * Lädt den spezifischen Feature-Song und synchronisiert ihn mit dem Cache.
 */
async function loadAndDisplayFeatureSongs() {
    try {
        // Fetch all the feature songs by their specific IDs
        const songIds = [
            'play_it_again',
            'one_pulse',
            'brainrot',
            'dancing_on_your_grave',
            'pathetique',
            'hurricane', 
            'surfing_girl_wild_and_free', 
            'fire_in_my_veins', 
            'black_cat_bones', 
            'southern_heat', 
            'southerns_eve', 
            'delta_blues', 
            'southern_belle'
        ];

        // Request all songs in parallel and wait for all to complete
        const results = await Promise.all(songIds.map(id => songService.getById(id)));

        // Create a flat array of all feature songs
        const featureData = results.flat();
        // const featureData = [...featureData1];
   
        if (featureData && featureData.length > 0) {
            // Die update-Methode triggert automatisch preCacheSongs
            featureCollection.update(featureData);
            console.log("SYSTEM: Feature Song synchronized.");
        }
    } catch (e) {
        console.error("SYSTEM: Feature Sync failed", e);
    }
}

/**
 * Corrected: Loads the specific podcasts and synchronizes them with the cache.
 */
async function loadAndDisplayPodcasts() {
    try {
        // 1. Fetch the data using the correct ID
        const podcast1 = await songService.getById('deepdive_1');
        const podcast2 = await songService.getById('deepdive_2');
        
        // 2. Combine them into one array
        const allPodcasts = [...podcast1, ...podcast2];

        if (allPodcasts.length > 0) {
            // 3. Use .update() instead of .add() (as defined in SongCollection.js)
            podcastCollection.update(allPodcasts);
            console.log("SYSTEM: Podcasts synchronized.");
        }      
    } catch (e) {
        console.error("SYSTEM: Podcast Sync failed", e);
    }
}

/**
 * Global bridge for the +/- buttons in the UI.
 * Connects the HTML onclick to the PickerDrum class instance.
 */
async function changeLimit(delta) {
    // We need the total song count from the service for the boundary check
    const allSongs = await songService.getAll();
    
    // Call the change method on our global drumPicker instance
    drumPicker.change(delta, allSongs.length);
}

/**
 * Aktualisiert alle Cache-Indikatoren für einen bestimmten Song auf der gesamten Seite.
 */
async function updateGlobalCacheStatus(songId) {
    const url = `${R2_DOMAIN}${songId}.mp3`;
    const cache = await caches.open(cacheName);
    const response = await cache.match(url);
    
    // Findet alle Punkte, egal in welcher Liste sie liegen
    const indicators = document.querySelectorAll(`[id$="-${songId}"]`);
    
    indicators.forEach(indicator => {
        if (response) {
            indicator.classList.remove('cache-offline', 'cache-syncing');
            indicator.classList.add('cache-online');
        } else {
            indicator.classList.remove('cache-online');
            indicator.classList.add('cache-offline');
        }
    });
}

/**
 * Global bridge for the "Fill Cache" button
 */
/**
 * Global bridge for the "Fill Cache" button
 * Jetzt mit direkter Rückmeldung an die Grids.
 */
async function fillCache() {
    const allVisibleSongs = [
        ...featureCollection.songs,
        ...archiveCollection.songs,
        ...podcastCollection.songs
    ];

    if (allVisibleSongs.length === 0) return;

    // Dubletten filtern (ID-basiert)
    const uniqueSongs = Array.from(new Set(allVisibleSongs.map(s => s.id)))
        .map(id => allVisibleSongs.find(s => s.id === id));

    for (let i = 0; i < uniqueSongs.length; i++) {
        const song = uniqueSongs[i];
        
        // 1. Technischer Download
        const success = await songService.cacheSong(song.id);
        
        if (success) {
            // 2. UI Update: Wir triggern die Status-Prüfung in beiden Kollektionen
            // Die Methode sucht in SongCollection.js nach der passenden ID
            await podcastCollection.updateCacheUIStatus(song.id, `cache-status-podcast-grid-${song.id}`);
            await featureCollection.updateCacheUIStatus(song.id, `cache-status-feature-song-grid-${song.id}`);
            await archiveCollection.updateCacheUIStatus(song.id, `cache-status-song-grid-${song.id}`);
            
            console.log(`SYSTEM: Neural Link synced for ${song.id}`);
        }
    }
}

/**
 * Global bridge for the "Purge Cache" button
 * Öffnet das Modal im neuen Design und setzt den Fokus auf Abbrechen.
 */
function purgeCache() {
    const modal = document.getElementById('purge-modal');
    const cancelBtn = document.getElementById('cancel-purge-btn');
    
    // Modal anzeigen
    modal.classList.remove('hidden');
    
    // Fokus auf den Cancel-Button setzen (für Tastaturbenutzer & Enter-Sicherheit)
    if (cancelBtn) {
        // Ein minimaler Delay stellt sicher, dass das Element im DOM aktiv ist
        setTimeout(() => {
            cancelBtn.focus();
        }, 50);
    }
}

/**
 * Schließt das Modal.
 */
function closePurgeModal() {
    const modal = document.getElementById('purge-modal');
    modal.classList.add('hidden');
}

/**
 * Führt den eigentlichen Löschvorgang aus.
 * Wird aufgerufen, wenn im Modal "Purge Archive" geklickt wird.
 */
async function confirmPurge() {
    // 1. Visuelles Feedback: Button deaktivieren/laden (optional)
    // 2. Löschvorgang durchführen
    const success = await songService.purgeCache();
    
    if (success) {
        // UI Refresh
        await loadAndDisplayFeatureSongs();
        await loadAndDisplayArchiveSongs();
        await loadAndDisplayPodcasts();
        
        // Modal schließen
        closePurgeModal();
        
        // Haptisches Feedback für Mobile
        if (window.navigator.vibrate) window.navigator.vibrate([50, 30, 50]);
        
        console.log("SYSTEM: Cache purged successfully.");
    }
}

/**
 * Global bridge for the Modal close button
 */
function closeDiaryModal() { diaryService.close(); }

/**
 * Global bridge for the Modal close button
 * Updated to terminate intros and speech synthesis.
 */
function closeSongModal() {
    const modal = document.getElementById('song-modal');
    
    // Nutze den neuen Kill-Switch des Players
    if (player) player.stopAll(); 

    // Restore background scrolling
    document.documentElement.style.scrollBehavior = 'auto';    
    document.body.style.overflow = '';
    document.body.style.position = '';
    document.body.style.top = '';
    document.body.style.width = '';

    window.scrollTo(0, scrollPosition);
    modal.classList.add('hidden');
    
    if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata = null;
    }
}

/**
 * Populates and shows the Modal UI (called by NeuralPlayer)
 */
/**
 * Populates and shows the Modal UI (called by NeuralPlayer)
 * Now includes dynamic artwork loading from the /artwork/ folder.
 */
function showSongModal(song, initialOpen = false) {
    const modal = document.getElementById('song-modal');
    const scrollContainer = modal.querySelector('.overflow-y-auto');

    if (initialOpen) {    
        document.body.style.overflow = 'hidden';
        document.body.style.position = 'fixed';
        document.body.style.top = `-${scrollPosition}px`;
        document.body.style.width = '100%';
    }

    modal.classList.remove('hidden');

    // Basic Info
    document.getElementById('modal-title').textContent = song.title;
    document.getElementById('modal-background').textContent = song.background;
    document.getElementById('modal-lyrics').textContent = song.lyrics;
    
    // --- ARTWORK LOGIC START ---
    // Select the title element to insert the image after it
    const titleEl = document.getElementById('modal-title');
    
    // Remove any existing artwork from previous modal opens
    const existingArt = document.getElementById('modal-artwork');
    if (existingArt) existingArt.remove();

    // Create the new image element
    const artImg = document.createElement('img');
    artImg.id = 'modal-artwork';
    artImg.src = `../artwork/${song.id}.webp`; // e.g., ../artwork/brooklyn.webp
    artImg.alt = `${song.title} Artwork`;
    artImg.className = 'w-full h-auto rounded-2xl mb-8 border border-white/10 shadow-2xl animate-in fade-in duration-700';
    artImg.style.contentVisibility = 'visible'; // Force loading even if initially off-screen
    
    // If the image fails to load (file not found), remove it to stay "as today"
    artImg.onerror = function() {
        this.remove();
    };

    // Insert directly under the title
    titleEl.insertAdjacentElement('afterend', artImg);
    // --- ARTWORK LOGIC END ---

    const tagsCont = document.getElementById('modal-tags');
    tagsCont.innerHTML = '';
    song.tags.forEach(tag => {
        const span = document.createElement('span');
        span.className = 'px-3 py-1 rounded-full bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-500 text-[9px] font-black uppercase tracking-widest';
        span.textContent = tag;
        tagsCont.appendChild(span);
    });

    if (window.lucide) lucide.createIcons();
    
    if (scrollContainer) {
        setTimeout(() => { 
            scrollContainer.scrollTo({ top: 0, behavior: 'instant' }); 
        }, 10); 
    }
}

// --- PLAYER UI BRIDGES ---
// These connect your existing HTML onclick handlers to the Player class
function playPreviousSong() { player.previous(); }
function playNextSong() { player.next(); }
function skipToStart() { player.audio.currentTime = 0; }
function skipToEnd() { if(player.audio.duration) player.audio.currentTime = player.audio.duration - 0.5; }

function handleRepeatToggle() {
    if (director.activeCollection) {
        director.activeCollection.config.repeat = !director.activeCollection.config.repeat;
        if (director.activeCollection.config.repeat){
            director.activeCollection.config.autoplay = false
        }
        player.syncUI(); // Aktualisiert die Button-Farben
    }
}

function handleModerationToggle() {
    if (director.activeCollection) {
        director.activeCollection.config.moderation = !director.activeCollection.config.moderation;
        player.syncUI();
    }
}

function handleAutoplayToggle() {
    if (director.activeCollection) {
        director.activeCollection.config.autoplay = !director.activeCollection.config.autoplay;
        if (director.activeCollection.config.autoplay){
            director.activeCollection.config.repeat = false
        }
        player.syncUI();
    }
}

// Global helper for Caching (used by SongCollection and NeuralPlayer)
async function forceCacheSong(url) {
    const cache = await caches.open(cacheName);
    const cachedResponse = await cache.match(url);
    if (cachedResponse) return; 

    try {
        const response = await fetch(`${url}?cb=${Date.now()}`, { mode: 'cors', cache: 'reload' });
        if (response.ok) await cache.put(url, response);
    } catch (err) {
        console.error("Neural Sync Error:", err);
    }
}

async function getCachedAudioUrl(url) {
    const cache = await caches.open(cacheName);
    const cachedResponse = await cache.match(url);
    if (cachedResponse) {
        const blob = await cachedResponse.blob();
        return URL.createObjectURL(blob);
    }
    return null;
}

/**
 * Aktualisiert das visuelle Feedback für den Online/Offline Status
 */
function updateOnlineStatus() {
    const menuItem = document.getElementById('status-menu-item');
    if (!menuItem) return;

    if (navigator.onLine) {
        // ONLINE: Fuchsia
        menuItem.textContent = 'Online';
        menuItem.classList.remove('text-neutral-500');
        menuItem.classList.add('text-fuchsia-500');
    } else {
        // OFFLINE: Grau
        menuItem.textContent = 'Offline';
        menuItem.classList.remove('text-fuchsia-500');
        menuItem.classList.add('text-neutral-500');
    }
}

// /**
//  * Schließt das Menü, wenn außerhalb geklickt wird.
//  */
// window.addEventListener('click', (e) => {
//     const mobileMenu = document.getElementById('mobile-menu');
//     const menuToggle = document.getElementById('menu-toggle');

//     // Wenn das Menü auf dieser Unterseite nicht existiert oder nicht offen ist -> Nichts tun
//     if (!mobileMenu || !menuToggle || !mobileMenu.classList.contains('active')) return;

//     // Klick-Kollisionsprüfung: Wenn der Klick NICHT im Menü UND NICHT auf dem Button war
//     if (!mobileMenu.contains(e.target) && !menuToggle.contains(e.target)) {
//         mobileMenu.classList.remove('active');
        
//         // Icon zurücksetzen
//         const icon = menuToggle.querySelector('i');
//         if (icon) icon.setAttribute('data-lucide', 'menu');
//         if (window.lucide) lucide.createIcons();
        
//         console.log("SYSTEM: Navigation Menu closed via outside interaction.");
//     }
// });

// // Schließen beim Klick auf einen mobilen Link
// document.querySelectorAll('.mobile-link').forEach(link => {
//     link.addEventListener('click', () => {
//         if (mobileMenu.classList.contains('active')) {
//             toggleMenu();
//         }
//     });
// });

// Update your initialization
window.addEventListener('DOMContentLoaded', async () => {
    // const menuToggle = document.getElementById('menu-toggle');
    // const mobileMenu = document.getElementById('mobile-menu');

    // if (!menuToggle || !mobileMenu) return;

    // // Klick auf den Burger-Button / Toggle-Button
    // menuToggle.addEventListener('click', (e) => {
    //     e.stopPropagation(); // WICHTIG: Verhindert das direkte Durchreichen an das 'window'
        
    //     mobileMenu.classList.toggle('active');
    //     const icon = menuToggle.querySelector('i');
    //     if (icon) {
    //         if (mobileMenu.classList.contains('active')) {
    //             icon.setAttribute('data-lucide', 'x');
    //         } else {
    //             icon.setAttribute('data-lucide', 'menu');
    //         }
    //     }
    //     if (window.lucide) lucide.createIcons();
    // });

    // // Schließen beim Klick auf einen mobilen Link im Menü
    // document.querySelectorAll('.mobile-link').forEach(link => {
    //     link.addEventListener('click', () => {
    //         if (mobileMenu.classList.contains('active')) {
    //             mobileMenu.classList.remove('active');
    //             const icon = menuToggle.querySelector('i');
    //             if (icon) icon.setAttribute('data-lucide', 'menu');
    //             if (window.lucide) lucide.createIcons();
    //         }
    //     });
    // }); 

    initNeuralFilterStrip();

    // 1. Erst die Services erstellen
    songService = new SongService();
    director = new NeuralDirector(songService);
    player = new NeuralPlayer(director);
    drumPicker = new PickerDrum('picker-drum', 'archive-count-input');
    diaryService = new DiaryService('diary-grid', 'diary-modal');

    // 2. Collections initialisieren mit ihren spezifischen Regeln
    archiveCollection = new SongCollection('song-grid', { 
        repeat: false,
        moderation: false, 
        autoplay: true, 
        allowServices: false 
    });
    
    featureCollection = new SongCollection('feature-song-grid', { 
        repeat: false,
        moderation: false, 
        autoplay: true, 
        allowServices: false 
    });
    
    podcastCollection = new SongCollection('podcast-grid', { 
        repeat: false,
        moderation: false, 
        autoplay: true, 
        allowServices: false 
    });    

    // Player UI synchronisieren
    player.syncUI();

    // --- NETWORK LOGIC ---
    window.addEventListener('online', updateOnlineStatus);
    window.addEventListener('offline', updateOnlineStatus);
    updateOnlineStatus();

    // Version im Footer setzen
    if (typeof updateFooterVersion === 'function') updateFooterVersion();

    // Mobile Viewport Height Fix
    const shadowHeight = () => {
        const doc = document.documentElement;
        doc.style.setProperty('--vh', `${window.innerHeight * 0.01}px`);
    };
    window.addEventListener('resize', shadowHeight);
    shadowHeight();

    // 3. SYSTEM INITIALIZATION (Asynchrone Datenbeschaffung)
    try {
        const allSongs = await songService.getAll();
        
        // WICHTIG: Drum-Picker initialisieren BEVOR die Archiv-Songs geladen werden!
        if (allSongs && allSongs.length) {
            drumPicker.init(allSongs.length, allSongs.length);
        }

        // Jetzt die Daten parallel/nacheinander laden
        await loadAndDisplayPodcasts(); 
        await loadAndDisplayFeatureSongs(); 
        await loadAndDisplayArchiveSongs(); 

        if (diaryService && typeof diaryService.refresh === 'function') {
            diaryService.refresh();
        }

    } catch (e) { 
        console.error("Neural System Initialization Failed:", e); 
    }

    // Global Key Bindings
    window.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            const songModal = document.getElementById('song-modal');
            const diaryModal = document.getElementById('diary-modal');
            if (songModal && !songModal.classList.contains('hidden')) closeSongModal();
            if (diaryModal && !diaryModal.classList.contains('hidden')) closeDiaryModal();
        }
    });

    // Speech Synthesis Stimmen-Fix
    if ('speechSynthesis' in window) {
        window.speechSynthesis.onvoiceschanged = () => {
            window.speechSynthesis.getVoices();
        };
        window.speechSynthesis.getVoices();
    }
});

// NEU: Den Observer erst binden, wenn wirklich alles im Browser gerendert wurde!
window.addEventListener('load', () => {
    setTimeout(() => {
        setupArchiveObserver();
    }, 100);
});

// Update your initialization
// addEL('DOMContentLoad', async () => {

//     // ERST HIER, wenn alle Skripte geladen sind, erstellen wir die Objekte
//     songService = new SongService();
//     director = new NeuralDirector(songService);
//     player = new NeuralPlayer(director);
//     drumPicker = new PickerDrum('picker-drum', 'archive-count-input');
//     diaryService = new DiaryService('diary-grid', 'diary-modal');

//     // Collections initialisieren mit ihren spezifischen Regeln
//     archiveCollection = new SongCollection('song-grid', { 
//         repeat: false,
//         moderation: false, 
//         autoplay: true, 
//         allowServices: false 
//     });
    
//     featureCollection = new SongCollection('feature-song-grid', { 
//         repeat: false,
//         moderation: false, 
//         autoplay: true, 
//         allowServices: false 
//     });
    
//     podcastCollection = new SongCollection('podcast-grid', { 
//         repeat: false,
//         moderation: false, 
//         autoplay: true, 
//         allowServices: false 
//     });    

//     // Update player UI
//     player.syncUI();

//     // --- NETWORK LOGIC ---
//     // Auf Änderungen während der Laufzeit reagieren
//     window.addEventListener('online', updateOnlineStatus);
//     window.addEventListener('offline', updateOnlineStatus);

//     // Einmaliger Check beim Start
//     updateOnlineStatus();

//     // Version update
//     updateFooterVersion();

//     // 1. MOBILE VIEWPORT HEIGHT FIX
//     // Prevents the address bar from cutting off the player on mobile browsers
//     const shadowHeight = () => {
//         const doc = document.documentElement;
//         doc.style.setProperty('--vh', `${window.innerHeight * 0.01}px`);
//     };
//     window.addEventListener('resize', shadowHeight);
//     shadowHeight();

//     // 2. SYSTEM INITIALIZATION
//     try {
//             const allSongs = await songService.getAll();
//             drumPicker.init(allSongs.length, allSongs.length);

//             // Sauberer Aufruf der modularen Funktionen
//             await loadAndDisplayPodcasts(); 
//             await loadAndDisplayFeatureSongs(); 
//             await loadAndDisplayArchiveSongs(); 

//             diaryService.refresh();

//             // INITIALER STATUS CHECK HIER EINFÜGEN
//             updateOnlineStatus();
//         } catch (e) { 
//             console.error("Neural System Initialization Failed:", e); 
//         }

//     // 3. GLOBAL KEYBOARD CONTROLS (Optional but recommended for window mode)
//     window.addEventListener('keydown', (e) => {
//         if (e.key === 'Escape') {
//             const songModal = document.getElementById('song-modal');
//             const diaryModal = document.getElementById('diary-modal');
//             if (!songModal.classList.contains('hidden')) closeSongModal();
//             if (!diaryModal.classList.contains('hidden')) closeDiaryModal();
//         }
//     });

//     // In Main.js am Ende des DOMContentLoaded Event-Listeners:
//     if ('speechSynthesis' in window) {
//         window.speechSynthesis.onvoiceschanged = () => {
//             window.speechSynthesis.getVoices();
//         };
//         window.speechSynthesis.getVoices(); // Initialer Trigger
//     }
// });

// function formatTimestamp(ts) {
//     if(!ts || ts.length < 13) return ts;
//     const y = ts.substring(0,4);
//     const m = ts.substring(4,6);
//     const d = ts.substring(6,8);
//     const h = ts.substring(9,11);
//     const min = ts.substring(11,13);
//     return `${d}.${m}.${y} // ${h}:${min}`;
// }

// --- UI UTILS ---


// window.addEventListener('scroll', () => {
//     const scrollY = window.scrollY;
//     const triggerPoint = 450;
//     const badge = document.getElementById('neural-status-badge'); // Neu hinzugefügt

//     if (scrollY < window.innerHeight) {
//         heroContent.style.transform = `translateY(${scrollY * -0.55}px)`;
//         heroImage.style.transform = `scale(${1 + (scrollY * 0.00015)})`;
//     }

//     if (scrollY > triggerPoint) {
//         navbar.classList.add('glass-nav', 'py-4'); 
//         navbar.classList.remove('py-10');
//         navTitle.classList.add('nav-visible'); 
//         navTitle.classList.remove('nav-hidden');
        
//         // Badge sichtbar machen
//         if (badge) {
//             badge.classList.add('nav-visible');
//             badge.classList.remove('nav-hidden');
//         }
//     } else {
//         navbar.classList.remove('glass-nav', 'py-4'); 
//         navbar.classList.add('py-10');
//         navTitle.classList.remove('nav-visible'); 
//         navTitle.classList.add('nav-hidden');

//         // Badge verstecken
//         if (badge) {
//             badge.classList.remove('nav-visible');
//             badge.classList.add('nav-hidden');
//         }
//     }
// });

// window.addEventListener('scroll', () => {
//     const navTitle = document.getElementById('nav-title');
//     const navbar = document.getElementById('navbar');
//     const heroContent = document.getElementById('hero-content');
//     const heroImage = document.getElementById('hero-image'); 

//     const scrollY = window.scrollY;
//     const triggerPoint = 450; // Reagiert angenehm früh beim Scrollen

//     if (scrollY < window.innerHeight) {
//         heroContent.style.transform = `translateY(${scrollY * -0.55}px)`;
//         heroImage.style.transform = `scale(${1 + (scrollY * 0.00015)})`;
//     }

//     if (scrollY > triggerPoint) {
//         navbar.classList.add('glass-nav'); 
//         // navbar.classList.remove('py-10');
//         navTitle.classList.add('nav-visible'); 
//         navTitle.classList.remove('nav-hidden');
//     } else {
//         navbar.classList.remove('glass-nav'); 
//         // navbar.classList.add('py-10');
//         navTitle.classList.remove('nav-visible'); 
//         navTitle.classList.add('nav-hidden');
//     }   
// });

window.addEventListener('scroll', () => {
    // Elemente erst abfragen, wenn das Scroll-Event feuert (Lazy Selection)
    const navTitle = document.getElementById('nav-title');
    const navbar = document.getElementById('navbar');
    const heroContent = document.getElementById('hero-content');
    const heroImage = document.getElementById('hero-image'); 

    // Sicherheits-Check: Wenn die Kern-Elemente (noch) nicht existieren, abbrechen
    if (!navbar || !navTitle) return;

    const scrollY = window.scrollY;
    const triggerPoint = 450; // Reagiert angenehm früh beim Scrollen

    // Parallax-Effekt für den Hero-Bereich (nur ausführen, wenn Elemente existieren)
    if (scrollY < window.innerHeight) {
        if (heroContent) heroContent.style.transform = `translateY(${scrollY * -0.55}px)`;
        if (heroImage) heroImage.style.transform = `scale(${1 + (scrollY * 0.00015)})`;
    }

    // Navbar-Klassen umschalten
    if (scrollY > triggerPoint) {
        navbar.classList.add('glass-nav'); 
        navTitle.classList.add('nav-visible'); 
        navTitle.classList.remove('nav-hidden');
    } else {
        navbar.classList.remove('glass-nav'); 
        navTitle.classList.remove('nav-visible'); 
        navTitle.classList.add('nav-hidden');
    }   
}, { passive: true }); // "passive: true" erhöht die Scroll-Performance spürbar!

// // Beispiel für die Toggle-Logik
// const menuToggle = document.getElementById('menu-toggle');
// const mobileMenu = document.getElementById('mobile-menu');

// /**
//  * Schaltet das Menü um und aktualisiert das Icon.
//  */
// function toggleMenu() {
//     mobileMenu.classList.toggle('active');
//     const icon = menuToggle.querySelector('i');
//     if (mobileMenu.classList.contains('active')) {
//         icon.setAttribute('data-lucide', 'x');
//     } else {
//         icon.setAttribute('data-lucide', 'menu');
//     }
//     if (window.lucide) lucide.createIcons();
// }

// // Klick auf den Button
// menuToggle.addEventListener('click', (e) => {
//     e.stopPropagation(); // Verhindert, dass der Klick sofort wieder das "Window-Click" Event auslöst
//     toggleMenu();
// });

// /**
//  * Schließt das Menü, wenn außerhalb geklickt wird.
//  */
// window.addEventListener('click', (e) => {
//     // Wenn das Menü offen ist UND der Klick nicht innerhalb des Menüs ODER auf dem Toggle-Button war
//     if (mobileMenu.classList.contains('active')) {
//         if (!mobileMenu.contains(e.target) && !menuToggle.contains(e.target)) {
//             toggleMenu();
//         }
//     }
// });

// // Schließen beim Klick auf einen Link (bereits vorhanden, optimiert)
// document.querySelectorAll('.mobile-link').forEach(link => {
//     link.addEventListener('click', () => {
//         if (mobileMenu.classList.contains('active')) {
//             toggleMenu();
//         }
//     });
// });

function injectSEOData(songs) {
    if (document.getElementById('json-ld-seo')) return;
    const schema = {
        "@context": "https://schema.org",
        "@type": "MusicPlaylist",
        "name": "Julia McCarren Neural Fusion Archive",
        "creator": {
            "@type": "Person",
            "name": "Julia McCarren",
            "jobTitle": "Solution Architect & Digital Artist",
            "sameAs": [
                "https://www.youtube.com/@JuliaMcCarren",
                "https://open.spotify.com/artist/1iEWsTM4vgUGVXOlky4Ujk"
            ]
        },
        "track": songs.map(s => ({
            "@type": "MusicRecording",
            "name": s.title,
            "url": "https://juliamccarren.com/"
        }))
    };
    const script = document.createElement('script');
    script.type = 'application/ld+json';
    script.text = JSON.stringify(schema);
    document.head.appendChild(script);
}

// // --- SERVICE WORKER REGISTRATION ---
// if ('serviceWorker' in navigator) {
//     window.addEventListener('load', () => {
//         // Optimiere diesen Teil in deiner Main.js:
//         navigator.serviceWorker.register('/sw.js', { scope: '/' }).then(reg => {
//             // FALL A: Ein Update wurde bereits im Hintergrund geladen und wartet
//             if (reg.waiting) {
//                 showUpdateBanner(reg);
//             }

//             // FALL B: Ein Update wird gerade erst gefunden
//             reg.addEventListener('updatefound', () => {
//                 const newWorker = reg.installing;
//                 newWorker.addEventListener('statechange', () => {
//                     if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
//                         showUpdateBanner(reg);
//                     }
//                 });
//             });
//         });
//     });
// }

// Globale Variable, um die SW-Registrierung seitenweit im Zugriff zu haben
let swRegistration = null;

// --- SERVICE WORKER REGISTRATION ---
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js', { scope: '/' })
            .then(reg => {
                // Speichere die Registrierung global
                swRegistration = reg;

                // 1. FALL: Es wartet bereits ein fertiges Update im Hintergrund
                if (reg.waiting) {
                    showUpdateBanner();
                }

                // 2. FALL: Ein Update wird im laufenden Betrieb gefunden
                reg.addEventListener('updatefound', () => {
                    const newWorker = reg.installing;
                    if (!newWorker) return;

                    newWorker.addEventListener('statechange', () => {
                        // Das Banner erst zeigen, wenn der Worker vollständig installiert (aber wartend) ist
                        if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                            showUpdateBanner();
                        }
                    });
                });
            })
            .catch(err => console.error("Service Worker Registration failed:", err));
    });
}

/**
 * Erzeugt das Update-Banner im DOM
 */
function showUpdateBanner() {
    if (document.getElementById('update-banner')) return;

    const banner = document.createElement('div');
    banner.id = 'update-banner';
    banner.innerHTML = `
        <span>Activate cache for new version</span>
        <i data-lucide="refresh-cw" style="width:14px; height:14px;"></i>
    `;
    document.body.appendChild(banner);
    if (window.lucide) lucide.createIcons();

    // Nutzt jetzt die dedizierte, fehlerfreie Aktivierungsfunktion
    banner.onclick = activateNewServiceWorker;
}

/**
 * Zwingt den wartenden Service Worker, sofort aktiv zu werden
 */
function activateNewServiceWorker() {
    // Sicherstellen, dass die Registrierung und der wartende Worker existieren
    if (swRegistration && swRegistration.waiting) {
        console.log("SYSTEM: Sending SKIP_WAITING to waiting Service Worker...");
        swRegistration.waiting.postMessage('SKIP_WAITING');
    } else {
        // Fallback: Falls die Referenz verloren ging, suchen wir sie direkt im Navigator
        navigator.serviceWorker.getRegistration().then(reg => {
            if (reg && reg.waiting) {
                console.log("SYSTEM: Fallback fallback triggered, sending SKIP_WAITING...");
                reg.waiting.postMessage('SKIP_WAITING');
            }
        });
    }
}

/**
 * Aktualisiert den Index Strip und fügt Mobile-Support hinzu
 */
/**
 * Aktualisiert den Index Strip und fügt Mobile-Support & Pfeil-Hinweis hinzu
 */
function updateNeuralIndexStrip(songs) {
    let strip = document.getElementById('neural-index-strip');
    if (!strip) {
        strip = document.createElement('div');
        strip.id = 'neural-index-strip';
        document.body.appendChild(strip);

        // Mobile-Fix: Toggle bei Touch
        strip.addEventListener('touchstart', (e) => {
            if (strip.offsetWidth < 50 || !strip.classList.contains('mobile-expanded')) {
                strip.classList.add('mobile-expanded');
            } else if (e.target === strip) {
                strip.classList.remove('mobile-expanded');
            }
        }, { passive: true });
    }

    strip.innerHTML = '';

    // 0. SLIDEOUT-PFEIL OBEN (Pfeil nach links)
    const handle = document.createElement('div');
    handle.className = 'strip-slide-handle';
    handle.innerHTML = '<i data-lucide="chevron-left"></i>';

    // WICHTIG: Klick- & Tap-Logik direkt auf dem Pfeil
    handle.addEventListener('click', (e) => {
        e.stopPropagation();
        strip.classList.toggle('mobile-expanded');
        if (window.navigator.vibrate) window.navigator.vibrate(5);
    });

    strip.appendChild(handle);

    // 1. SONG-EINTRÄGE
    songs.forEach((song, index) => {
        const item = document.createElement('div');
        item.className = 'index-item';
        item.textContent = song.title.substring(0, 20);
        
        item.onclick = (e) => {
            e.stopPropagation();
            const cards = document.querySelectorAll('#song-grid > div');
            if (cards[index]) {
                cards[index].scrollIntoView({ behavior: 'smooth', block: 'center' });
                cards[index].classList.add('ring-2', 'ring-fuchsia-500');
                setTimeout(() => cards[index].classList.remove('ring-2', 'ring-fuchsia-500'), 2000);
            }
            strip.classList.remove('mobile-expanded');
        };
        strip.appendChild(item);
    });

    // Icons neu aufbauen
    if (window.lucide) {
        lucide.createIcons();
    }
}

/**
 * Schließt die Leiste, wenn man irgendwo anders hinklickt
 */
window.addEventListener('touchstart', (e) => {
    const strip = document.getElementById('neural-index-strip');
    if (strip && !strip.contains(e.target)) {
        strip.classList.remove('mobile-expanded');
    }
}, {passive: true});

/**
 * Blendet die Leiste nur im Archiv-Bereich ein
 */
function setupArchiveObserver() {
    const archiveSection = document.getElementById('archives');
    const rightStrip = document.getElementById('neural-index-strip');
    const leftStrip = document.getElementById('neural-filter-strip');
    
    if (!archiveSection) return;

    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                if (rightStrip) rightStrip.classList.add('visible');
                if (leftStrip) leftStrip.classList.add('visible');
            } else {
                if (rightStrip) {
                    rightStrip.classList.remove('visible');
                    rightStrip.classList.remove('mobile-expanded');
                }
                if (leftStrip) leftStrip.classList.remove('visible');
            }
        });
    }, { 
        threshold: 0,
        rootMargin: "-10% 0px -10% 0px"
    });

    observer.observe(archiveSection);
}

/**
 * Lädt eine neue Zufallsauswahl an Songs für das Archiv,
 * rendert das Grid neu und synchronisiert die Playlist im Director.
 */
async function loadAndDisplayArchiveSongs() {
    // 1. Suchfeld im UI finden und leeren, um Konsistenz zu wahren
    const searchInput = document.querySelector('input[oninput^="handleArchiveSearch"]');
    if (searchInput) {
        searchInput.value = '';
    }

    // 2. Bestehende Highlights (Suchergebnisse) im Grid entfernen
    if (archiveCollection && archiveCollection.container) {
        const cards = Array.from(archiveCollection.container.children);
        const highlightClasses = ['ring-2', 'ring-fuchsia-500', 'bg-neutral-900/60'];
        cards.forEach(card => card.classList.remove(...highlightClasses));
    }

    try {
        // 3. Neue Songs basierend auf dem Picker-Limit vom Service holen
        const limit = drumPicker.value;
        const randomSongs = await songService.getRandom(limit);

        // 4. Die SongCollection mit den neuen Daten füttern (löst das Rendering aus)
        archiveCollection.update(randomSongs);

        // NEU: Index Strip synchronisieren
        updateNeuralIndexStrip(randomSongs);    
        setupArchiveObserver(); // Observer sicherheitshalber neu binden        

        /**
         * 5. DIREKTOR-SYNCHRONISATION
         * Falls das Archiv die aktuell aktive Collection im Director ist,
         * müssen wir die interne Playlist des Directors aktualisieren,
         * damit die "Next"-Logik die neuen Songs kennt.
         */
        if (director && director.activeCollection === archiveCollection) {
            director.playlist = randomSongs;
            console.log("SYSTEM: Director playlist synchronized with new Archive selection.");
        }

        console.log(`SYSTEM: ${randomSongs.length} new neural patterns loaded into archive.`);
    } catch (error) {
        console.error("SYSTEM: Failed to reshuffle archive:", error);
    }
}

// Sobald der neue SW die Kontrolle übernimmt -> Refresh
let refreshing = false;
navigator.serviceWorker.addEventListener('controllerchange', () => {
    if (!refreshing) {
        // Wir hängen einen kurzen Hash an die URL an, NUR für diesen einen Reload
        // Das zwingt den Browser, den Memory-Cache der Scripte komplett zu verwerfen.
        window.location.href = window.location.origin + window.location.pathname + '?v=' + Date.now();
        window.location.reload();
        refreshing = true;
    }
});

/**
 * Filtert das Archiv nach einem spezifischen Tag und aktualisiert das Grid.
 * @param {string} targetTag - Der Tag, nach dem gefiltert werden soll
 */
async function filterArchiveByTag(targetTag) {
    if (!songService || !archiveCollection) return;

    try {
        // 1. Alle Songs vom Service abrufen
        const allSongs = await songService.getAll();
        
        // 2. Nach dem spezifischen Tag filtern (Groß-/Kleinschreibung ignorieren)
        const filteredSongs = allSongs.filter(song => 
            song.tags && song.tags.some(tag => tag.toLowerCase() === targetTag.toLowerCase())
        );

        if (filteredSongs.length === 0) {
            console.warn(`SYSTEM: No songs found with tag "${targetTag}"`);
            return;
        }

        // 3. Archiv-Collection aktualisieren
        archiveCollection.update(filteredSongs);

        // 4. Index Strip und Observer aktualisieren
        updateNeuralIndexStrip(filteredSongs);
        setupArchiveObserver();

        // 5. Director-Playlist synchronisieren, falls aktiv
        if (director && director.activeCollection === archiveCollection) {
            director.playlist = filteredSongs;
        }

        console.log(`SYSTEM: Filtered archive by tag "${targetTag}" (${filteredSongs.length} songs loaded).`);
    } catch (error) {
        console.error("SYSTEM: Failed to filter archive by tag:", error);
    }
}

/**
 * Lädt die 10 aktuellsten Songs (basierend auf der ursprünglichen Reihenfolge im Service) und zeigt sie im Archiv an.
 */
async function loadLatestArchiveSongs() {
    if (!songService || !archiveCollection) return;

    try {
        // 1. Alle Songs vom Service abrufen (die Grundreihenfolge entspricht in der Regel dem neuesten Stand)
        const allSongs = await songService.getAll();
        
        // 2. Die ersten 10 Songs extrahieren
        const latestSongs = allSongs.slice(0, 10);

        if (latestSongs.length === 0) return;

        // 3. Archiv-Collection aktualisieren
        archiveCollection.update(latestSongs);

        // 4. Index Strip und Observer aktualisieren
        updateNeuralIndexStrip(latestSongs);
        setupArchiveObserver();

        // 5. Director-Playlist synchronisieren, falls aktiv
        if (director && director.activeCollection === archiveCollection) {
            director.playlist = latestSongs;
        }

        console.log(`SYSTEM: Loaded the 10 latest songs into the archive.`);
    } catch (error) {
        console.error("SYSTEM: Failed to load latest songs:", error);
    }
}

/**
 * Scrollt direkt zum Beginn der Liedliste unterhalb der Sticky Controls
 */
/**
 * Scrollt zum Archiv-Grid unter Berücksichtigung der Sticky Controls und Navbar
 */
function scrollToSongGrid() {
    const grid = document.getElementById('song-grid');
    const stickyControls = document.querySelector('.archive-sticky-controls');
    const navbar = document.getElementById('navbar');

    if (!grid) return;

    // Höhen der fixierten und sticky Elemente ermitteln
    const navHeight = navbar ? navbar.getBoundingClientRect().height : 0;
    const controlsHeight = stickyControls ? stickyControls.getBoundingClientRect().height : 0;
    
    // Zusätzlicher Sicherheitsabstand
    const buffer = 16; 
    
    // Absolute Zielposition im Dokument berechnen
    const targetY = grid.getBoundingClientRect().top + window.scrollY - navHeight - controlsHeight - buffer;

    window.scrollTo({
        top: Math.max(0, targetY),
        behavior: 'smooth'
    });
}

/**
 * Initialisiert den linken Filter-Strip für Suchfeld, Tags, Sortierung & Filter
 */
function initNeuralFilterStrip() {
    let strip = document.getElementById('neural-filter-strip');
    if (strip) return;

    strip = document.createElement('div');
    strip.id = 'neural-filter-strip';

    // Mobile-Fix: Ein- und Ausklappen per Tap
    strip.addEventListener('touchstart', (e) => {
        if (!strip.classList.contains('mobile-expanded')) {
            strip.classList.add('mobile-expanded');
        } else if (e.target === strip) {
            strip.classList.remove('mobile-expanded');
        }
    }, { passive: true });

    // 0. SLIDEOUT-PFEIL OBEN (Pfeil nach rechts)
    const handle = document.createElement('div');
    handle.className = 'strip-slide-handle';
    handle.innerHTML = '<i data-lucide="chevron-right"></i>';

    // WICHTIG: Klick- & Tap-Logik direkt auf dem Pfeil
    handle.addEventListener('click', (e) => {
        e.stopPropagation();
        strip.classList.toggle('mobile-expanded');
        if (window.navigator.vibrate) window.navigator.vibrate(5);
    });    

    strip.appendChild(handle);

    // 1. NEUES SUCHFELD
    const searchContainer = document.createElement('div');
    searchContainer.className = 'filter-strip-search-box';
    searchContainer.innerHTML = `
        <i data-lucide="search" class="filter-strip-search-icon"></i>
        <input 
            type="text" 
            placeholder="Search archive..." 
            class="filter-strip-search-input"
            oninput="handleArchiveSearch(this.value)"
        />
    `;
    // Verhindert das versehentliche Zuklappen bei Klick/Fokus im Suchfeld
    searchContainer.querySelector('input').addEventListener('click', (e) => e.stopPropagation());
    searchContainer.querySelector('input').addEventListener('touchstart', (e) => e.stopPropagation());
    strip.appendChild(searchContainer);

    // 2. SORTIER-BUTTONS
    const sortRow = document.createElement('div');
    sortRow.className = 'filter-strip-sort-row';

    const sortButtons = [
        { 
            icon: 'arrow-down-a-z', 
            label: 'A–Z', 
            title: 'Sort A to Z', 
            action: () => {
                sortArchiveSongs('asc');
                scrollToSongGrid();
            } 
        },
        { 
            icon: 'arrow-down-z-a', 
            label: 'Z–A', 
            title: 'Sort Z to A', 
            action: () => {
                sortArchiveSongs('desc');
                scrollToSongGrid();
            } 
        }
    ];

    sortButtons.forEach(item => {
        const btn = document.createElement('button');
        btn.className = 'filter-strip-btn filter-strip-btn-sm';
        btn.title = item.title;
        btn.innerHTML = `<i data-lucide="${item.icon}"></i><span>${item.label}</span>`;
        btn.onclick = (e) => {
            e.stopPropagation();
            item.action();
            strip.classList.remove('mobile-expanded');
            if (window.navigator.vibrate) window.navigator.vibrate(5);
        };
        sortRow.appendChild(btn);
    });
    strip.appendChild(sortRow);

    // 3. TRENNLINIE
    const divider = document.createElement('div');
    divider.className = 'filter-strip-divider';
    strip.appendChild(divider);

    // 4. FILTER-BUTTONS
    const filters = [
        { 
            label: 'Made in Italy', 
            action: async () => {
                await filterArchiveByTag('Made in Italy');
                scrollToSongGrid();
            } 
        },
        { 
            label: 'Southern Saga', 
            action: async () => {
                await filterArchiveByTag('Southern Saga');
                scrollToSongGrid();
            } 
        },
        { 
            label: 'Radio Trash', 
            action: async () => {
                await filterArchiveByTag('Radio Trash');
                scrollToSongGrid();
            } 
        },        
        { 
            label: 'Last 10', 
            action: async () => {
                await loadLatestArchiveSongs();
                scrollToSongGrid();
            } 
        }
    ];

    filters.forEach(item => {
        const btn = document.createElement('button');
        btn.className = 'filter-strip-btn';
        btn.textContent = item.label;
        btn.onclick = (e) => {
            e.stopPropagation();
            item.action();
            strip.classList.remove('mobile-expanded');
            if (window.navigator.vibrate) window.navigator.vibrate(5);
        };
        strip.appendChild(btn);
    });

// 5. TRENNLINIE FÜR CACHE-AKTIONEN
    const cacheDivider = document.createElement('div');
    cacheDivider.className = 'filter-strip-divider';
    strip.appendChild(cacheDivider);

    // 6. CACHE-BUTTONS (Reiner Text, untereinander)
    // Button: Fill Cache
    const fillBtn = document.createElement('button');
    fillBtn.className = 'filter-strip-btn filter-strip-btn-fill';
    fillBtn.textContent = 'Fill Cache';
    fillBtn.onclick = async (e) => {
        e.stopPropagation();
        if (window.navigator.vibrate) window.navigator.vibrate(5);
        await fillCache();
        strip.classList.remove('mobile-expanded');
    };
    strip.appendChild(fillBtn);

    // Button: Purge Cache
    const purgeBtn = document.createElement('button');
    purgeBtn.className = 'filter-strip-btn filter-strip-btn-purge';
    purgeBtn.textContent = 'Purge Cache';
    purgeBtn.onclick = (e) => {
        e.stopPropagation();
        if (window.navigator.vibrate) window.navigator.vibrate(5);
        purgeCache(); // Öffnet das Bestätigungs-Modal
        strip.classList.remove('mobile-expanded');
    };
    strip.appendChild(purgeBtn);

    document.body.appendChild(strip);

    if (window.lucide) {
        lucide.createIcons();
    }
}

/**
 * Schließt beide Leisten beim Tippen ins Leere auf Mobilgeräten
 */
window.addEventListener('touchstart', (e) => {
    const rightStrip = document.getElementById('neural-index-strip');
    const leftStrip = document.getElementById('neural-filter-strip');

    if (rightStrip && !rightStrip.contains(e.target)) {
        rightStrip.classList.remove('mobile-expanded');
    }
    if (leftStrip && !leftStrip.contains(e.target)) {
        leftStrip.classList.remove('mobile-expanded');
    }
}, { passive: true });

