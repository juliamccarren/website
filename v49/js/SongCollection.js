...
class SongCollection {
    ...
    render() {
        ...
        setTimeout(() => {
            ...
            this.songs.forEach((song, index) => {
                ...
                // Wichtig: overflow-hidden damit der Glow nicht über die abgerundeten Ecken hinausragt
                card.className = 'group relative bg-neutral-900/80 border border-white/5 p-8 rounded-3xl hover:bg-neutral-900/60 transition-all duration-500 overflow-hidden animate-in fade-in slide-in-from-bottom-4';
                ...
                card.innerHTML = `
                    <div class="absolute inset-0 z-0 opacity-20 group-hover:opacity-40 group-hover:scale-110 transition-all duration-700 bg-cover bg-center" 
                        style="background-image: url('${artworkUrl}');">
                    </div>
                    
                    <div class="absolute inset-0 z-1 bg-gradient-to-t from-black via-black/40 to-transparent"></div>

                    <div class="absolute bottom-0 left-0 w-full overflow-hidden h-6 pointer-events-none">
                        <div id="${statusId}" class="cache-status-edge cache-offline-edge"></div>
                    </div>
                ...
                // Artwork check
                const bgLayer = card.querySelector('.absolute.inset-0.z-0');
                const imgTester = new Image();
                imgTester.src = artworkUrl;
                imgTester.onerror = () => bgLayer.remove();
    ...
    async updateCacheUIStatus(songId, statusId) {
        ...
        const cache = await caches.open(cacheName); // Nutzt den globalen cacheName
