...
// Hilfsfunktion zum sauberen Umschalten von Klassen und Icons
    function toggleMenu() {
        if (!mobileMenu || !menuToggle) return;
        
        mobileMenu.classList.toggle('active');
        
        // Icon-Umschaltung (Burger zu X und zurück)
...
// Hilfsfunktion zum sauberen Umschalten von Klassen und Icons
    ...
    if (menuToggle && mobileMenu) {
        menuToggle.addEventListener('click', (e) => {
            e.stopPropagation(); // Verhindert das Durchreichen an das 'window'
            toggleMenu();
        });

        // WICHTIG: Holt alle Links, aber ignoriert den Essays-Akkordeon-Button explizit!
        mobileMenu.querySelectorAll('.mobile-link:not(#mobile-essays-toggle)').forEach(link => {
        ...
        // Globaler Klick-Listener für Klicks außerhalb (Schließen des Menüs)
        window.addEventListener('click', (e) => {
            if (!mobileMenu.classList.contains('active')) return;

            // Wenn der Klick NICHT im Menü und NICHT auf dem Button war -> Schließen
