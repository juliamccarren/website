...
/**
 * Erstellt oder aktualisiert die Index-Leiste basierend auf den geladenen Songs
 */
function updateNeuralIndexStrip(songs) {
    ...
    if (!strip) {
        ...
        // NEU: Touch-Toggle für Mobile
        strip.onclick = (e) => {
            if (e.target === strip) {
                strip.classList.toggle('expand-mobile');
            }
        };
    ...
    songs.forEach((song, index) => {
        ...
        item.onclick = () => {
            ...
            if (cards[index]) {
                ...
                cards[index].classList.add('ring-2', 'ring-fuchsia-500', 'bg-neutral-900/60');
                setTimeout(() => cards[index].classList.remove('ring-2', 'ring-fuchsia-500', 'bg-neutral-900/60'), 2000);
            }
        };
