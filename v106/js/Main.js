...
function initNeuralFilterStrip() {
    ...
    const sortButtons = [
        { icon: 'arrow-down-a-z', label: 'A–Z', title: 'Sort A to Z', action: () => sortArchiveSongs('asc') },
        { icon: 'arrow-down-z-a', label: 'Z–A', title: 'Sort Z to A', action: () => sortArchiveSongs('desc') }
    ];
    ...
    // 4. FILTER-BUTTONS
    const filters = [
        { label: 'Made in Italy', action: () => filterArchiveByTag('Made in Italy') },
        { label: 'Southern Saga', action: () => filterArchiveByTag('Southern Saga') },
        { label: 'Last 10', action: () => loadLatestArchiveSongs() }
    ];
